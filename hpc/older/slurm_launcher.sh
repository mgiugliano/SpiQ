...
for f in $Files; do   						# For each *.mcd file in inputDir..
	...
	sbatch --partition=wide1 -N1 --time=$MAXTIME --cpus-per-task=$NCORES --mem=320000 --job-name=QSpikeTools_$fileName --output=$logDir/qspiketools.out --error=$logDir/qspiketools.err --chdir=$(pwd) start.sh $1 $f 0
...
for f in $Files; do   						# For each *.mcd file in inputDir..
	...
	#sbatch --partition=wide1 -N1 --time=$MAXTIME --cpus-per-task=$NCORES --mem=320000 --job-name=QSpikeTools_$fileName --output=$logDir/qspiketools.out --error=$logDir/qspiketools.err --chdir=$(pwd) start.sh $1 $f 1
	sbatch --partition=wide1 -N1 --time=4:00:00 --cpus-per-task=40 --mem=320000 --mail-type=END,FAIL --mail-user=mmanzati@sissa.it --job-name=QSpikeTools_$fileName --output=$logDir/qspiketools.out --error=$logDir/qspiketools.err --chdir=$(pwd) start.sh $1 $f 1
