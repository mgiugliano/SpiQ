#!/bin/bash
#
# QSpike Tools 2.0 - QSUB-LAUNCHER
# 
# AUTHORS:
...
# Launches QSpikeTools in a cluster using qsub. First it separates all 
# the recordings found in *.h5 files in different *.h5qspike files. Then 
# it launches one qsub job per each *.mcd and *.h5qspike file.
#
# INPUT:
...
for f in $Files; do   						# For each *.mcd file in inputDir..
	...
	qsub -l walltime=$MAXTIME -l nodes=1:ppn=$NCORES -N QSpikeTools_$fileName -o $logDir/qspiketools.out -e $logDir/qspiketools.err -d $(pwd) start.sh -F "$1 $f 0"
done
...
for f in $Files; do   						# For each *.mcd file in inputDir..
	...
	qsub -l walltime=$MAXTIME -l nodes=1:ppn=$NCORES -N QSpikeTools_$fileName -o $logDir/qspiketools.out -e $logDir/qspiketools.err -d $(pwd) start.sh -F "$1 $f 1"
done
