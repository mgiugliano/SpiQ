...
for f in $Files; do   						# For each *.mcd file in inputDir..
	...
	#sbatch --partition=wide1 -N1 --time=2:00:00 --cpus-per-task=$NCORES --mem=320000 --job-name=QSpikeTools_$fileName --output=$logDir/qspiketools.out --error=$logDir/qspiketools.err --chdir=$(pwd) start.sh $1 $f 1
...
# wall clock time for a 30 min rec is roughly 2h
